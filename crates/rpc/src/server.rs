...
mod tests {
    ...
    #[tokio::test]
    async fn parameterless_subscriptions_reject_extra_params_without_leaking_capacity() {
        let db = Arc::new(MemoryDb::new());
        let chain_store = Arc::new(ChainStore::new(db.clone()));
        let world_state = Arc::new(parking_lot::RwLock::new(WorldState::new(db)));
        let tx_pool = Arc::new(TxPool::new(MempoolConfig::default()));
        ...
        for sub_type in ["newHeads", "syncing"] {
            for _ in 0..17 {
                let result = client
                    .subscribe::<serde_json::Value, _>(
                        "eth_subscribe",
                        rpc_params![sub_type, false],
                        "eth_unsubscribe",
                    )
                    .await;
                assert!(
                    matches!(result, Err(ClientError::Call(ref err)) if err.code() == -32602),
                    "parameterless subscription accepted extra params: {result:?}"
                );
            }
        }
