...
mod tests {
    ...
    async fn protocol_activation_scheduling_is_independent_atomic_and_persistent() {
        ...
        config.algorithm_timelock_activation_height = Some(0);
        assert!(initialize_chain(
            Arc::clone(&store),
            &config,
            dir.path(),
        ...
        config.algorithm_proposal_staging_height = Some(0);
        assert!(initialize_chain(
            Arc::clone(&store),
            &config,
            dir.path(),
        ...
        .is_err());
        assert_eq!(store.scan_prefix(b"").unwrap(), before);
        config.algorithm_proposal_staging_height = Some(7);
        config.algorithm_voting_window_activation_height = Some(0);
        assert!(initialize_chain(
            Arc::clone(&store),
            &config,
            dir.path(),
        ...
        .is_err());
        assert_eq!(store.scan_prefix(b"").unwrap(), before);
        config.algorithm_voting_window_activation_height = Some(8);
        initialize_chain(
            Arc::clone(&store),
            &config,
            dir.path(),
        ...
        config.algorithm_deprecation_height = Some(10);
        config.algorithm_session_deprecation_height = Some(0);
        let before = store.scan_prefix(b"").unwrap();
        assert!(initialize_chain(
            Arc::clone(&store),
            &config,
        ...
        let chain = ChainStore::new(Arc::clone(&store));
        let persisted = chain.get_chain_config().unwrap().unwrap();
        assert_eq!(persisted.algorithm_session_deprecation_height, Some(11));
        let unchanged = store.scan_prefix(b"").unwrap();
        for conflicting in [None, Some(12)] {
            config.algorithm_session_deprecation_height = conflicting;
            assert!(initialize_chain(
                Arc::clone(&store),
                &config,
                dir.path(),
        ...
        .is_err());
        assert_eq!(store.scan_prefix(b"").unwrap(), before);
        config.session_registered_root_height = Some(12);
        initialize_chain(
            Arc::clone(&store),
            &config,
            dir.path(),
        ...
        let chain = ChainStore::new(Arc::clone(&store));
        let persisted = chain.get_chain_config().unwrap().unwrap();
        assert_eq!(persisted.session_registered_root_height, Some(12));
        let unchanged = store.scan_prefix(b"").unwrap();
        for conflicting in [None, Some(13)] {
            config.session_registered_root_height = conflicting;
            assert!(initialize_chain(
                Arc::clone(&store),
                &config,
                dir.path(),
        ...
        assert_eq!(persisted.algorithm_proposal_staging_height, Some(7));
        assert_eq!(
            persisted.algorithm_voting_window,
            Some(shell_storage::AlgorithmVotingWindow {
                activation_height: 8,
                block_time_secs: 2
            })
        );
        ...
        config.algorithm_quorum_activation_height = Some(6);
        for conflicting in [None, Some(8)] {
            config.algorithm_proposal_staging_height = conflicting;
            assert!(initialize_chain(
                Arc::clone(&store),
                &config,
                dir.path(),
        ...
        config.algorithm_proposal_staging_height = Some(7);
        for conflicting in [None, Some(9)] {
            config.algorithm_voting_window_activation_height = conflicting;
            assert!(initialize_chain(
                Arc::clone(&store),
                &config,
                dir.path(),
        ...
        config.algorithm_voting_window_activation_height = Some(8);
        match &mut config.consensus {
            shell_genesis::ConsensusConfig::PoA {
                block_time_secs, ..
            }
            | shell_genesis::ConsensusConfig::WPoA {
                block_time_secs, ..
            } => *block_time_secs = 3,
        }
        assert!(initialize_chain(
